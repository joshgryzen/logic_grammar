...
`python get_model_response_with_pipe_strong_negation.py --meta-llama/Llama-3.2-3B-Instruct`

To run the baseline comparison, simpley run: 

`python get_model_response_baseline.py --meta-llama/Llama-3.2-3B-Instruct`
...
**get_model_response.py** uses the outline library to restrict the LLM output to adhere to a grammar. 
